	...
	public class TMP_TextSelector_A : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IEventSystemHandler
	{
		...
		private void LateUpdate()
		{
			...
			if (this.m_isHoveringObject)
			{
				...
				if (num != -1 && num != this.m_lastCharIndex && (Input.GetKey(304) || Input.GetKey(303)))
				{
					...
					Color32 color;
					color..ctor((byte)Random.Range(0, 255), (byte)Random.Range(0, 255), (byte)Random.Range(0, 255), byte.MaxValue);
				...
				if (num3 != -1 && num3 != this.m_lastWordIndex)
				{
					...
					Color32 color2;
					color2..ctor((byte)Random.Range(0, 255), (byte)Random.Range(0, 255), (byte)Random.Range(0, 255), byte.MaxValue);
