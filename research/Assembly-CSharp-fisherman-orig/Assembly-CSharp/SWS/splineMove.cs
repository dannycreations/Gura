...
namespace SWS
{
	...
	public class splineMove : MonoBehaviour, ISplineMover
	{
		...
		public void SetPath(PathManager newPath, float timePrc = 0f)
		{
			this.SetPath(newPath, timePrc, this.speed);
		}

		public void SetPath(PathManager newPath, float timePrc, float movementSpeed)
		{
			this.speed = movementSpeed;
			this.Stop();
			this.pathContainer = newPath;
			this.StartMove();
			float num = timePrc * TweenExtensions.Duration(this.tween, true);
			TweenExtensions.ForceInit(this.tween);
			TweenExtensions.Goto(this.tween, num, false);
			TweenExtensions.Play<Tweener>(this.tween);
		}
		...
		public void ChangeSpeed(float value)
		{
			...
			if (this.timeValue == splineMove.TimeValue.speed)
			{
				num = value / this.originSpeed;
			}
			else
			{
				num = this.originSpeed / value;
			}
			this.speed = value;
			if (this.tween != null)
			{
				this.tween.timeScale = num;
			}
		}
