...
public class PremiumShopRetailHandler : ActivityStateControlled
{
	...
	protected override void SetHelp()
	{
		...
		if (productCategory != null)
		{
			...
			ClientDebugHelper.Log(ProfileFlag.ProductDelivery, string.Format("PremiumShopRetailHandler DLC - productsIds:{0}", string.Join(",", productsIds.Select((int p) => p.ToString()).ToArray<string>())));
			...
			list.ForEach(delegate(StoreProduct p)
			{
				ClientDebugHelper.Log(ProfileFlag.ProductDelivery, string.Format("PremiumShopRetailHandler ADD - Price:{0} PriceString:{1} DiscountPrice:{2} ProductId:{3} ProductCurrency:{4} ProductSkuId:{5}", new object[] { p.Price, p.PriceString, p.DiscountPrice, p.ProductId, p.ProductCurrency, p.ProductSkuId }));
			});
