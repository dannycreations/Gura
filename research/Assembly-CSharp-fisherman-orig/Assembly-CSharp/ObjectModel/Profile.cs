		...
		public bool HasPremium
		{
			get
			{
				...
				if (this.SubscriptionId != null && this.SubscriptionEndDate != null)
				{
					...
					flag = TimeHelper.UtcTime() < subscriptionEndDate;
				}
				else
				{
					flag = false;
		...
		public void DeleteSubscriptions(IEnumerable<Subscription> subscriptionsToDelete)
		{
			using (IEnumerator<Subscription> enumerator = subscriptionsToDelete.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					else if (s.IsPaidUnlock)
					{
						this.PaidLockRemovals.RemoveAll((PaidLockRemoval r) => r.Ponds != null && r.Ponds.Length == 1 && r.Ponds[0] == s.PondId);
						if (s.Licenses != null)
						{
							int[] licenses = s.Licenses;
							for (int i = 0; i < licenses.Length; i++)
							{
								int licenseId = licenses[i];
								this.Licenses.RemoveAll((PlayerLicense l) => l.LicenseId == licenseId);
							}
						}
						if (s.Silver > 0)
						{
							this.SilverCoins -= (double)s.Silver;
						}
						if (s.Gold > 0)
						{
							this.GoldCoins -= (double)s.Gold;
						}
					}
					else
					{
