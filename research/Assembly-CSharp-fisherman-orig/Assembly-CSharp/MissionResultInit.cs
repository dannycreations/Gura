...
public class MissionResultInit : DayResultInit
{
	...
	{
		base.Awake();
		string text = "AchievmentRewardCaption";
		this._leftRuller.sizeDelta = new Vector2(308f, this._leftRuller.rect.height);
		this._achivsPanel.sizeDelta = new Vector2(900f, this._achivsPanel.rect.height);
		this._challenges.anchoredPosition = new Vector2(-135f, this._challenges.anchoredPosition.y);
		this._rightRuller.sizeDelta = new Vector2(760f, this._rightRuller.rect.height);
		this._rightRuller.anchoredPosition = new Vector2(this._rightRuller.anchoredPosition.x - 185f, this._rightRuller.anchoredPosition.y);
		this._achivementsCaption.anchoredPosition = new Vector2(this._achivementsCaption.anchoredPosition.x - 185f, this._achivementsCaption.anchoredPosition.y);
		this._textRewards.text = ScriptLocalization.Get(text);
	}

	protected override float NoPremWndHeight()
	{
		return this.Window.rect.height;
	}

	protected override float NoPremContentY0()
	{
		return 80f;
	}
	...
	private const float LeftRullerWidth = 308f;

	private const float RightRullerWidth = 760f;

	private const float AchivsPanelWidth = 900f;

	private const float AchivementsCaptionOffsetX = 185f;

	private const float ChallengesX0 = -135f;
