...
public class FriendsHandlerNew : ActivityStateControlled
{
	...
	{
		string text = ((player == null) ? "Null" : string.Format("UserName:{0} UserId:{1} Status:{2}", player.UserName, player.UserId, player.Status));
		LogHelper.Log("{0}:OnFriendshipAdded [{1}]", new object[]
		{
			base.GetType().Name,
			text
		});
	...
	{
		string text = ((player == null) ? "Null" : string.Format("UserName:{0} UserId:{1} Status:{2}", player.UserName, player.UserId, player.Status));
		LogHelper.Log("{0}:OnFriendshipRemoved [{1}]", new object[]
		{
			base.GetType().Name,
			text
		});
