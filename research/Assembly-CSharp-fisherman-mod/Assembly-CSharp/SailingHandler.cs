...
public class SailingHandler : MonoBehaviour, IBoatWithEngineMinigameView
{
	...
	private void UpdateSailingInfo()
	{
		if (this._isBoarded)
		{
			...
			this.EngineAction.SetActive(!GameFactory.Player.IsAnchored && GameFactory.Player.IsEnginePresent && !GameFactory.Player.IsTrollingPossible);
			...
			this.TrollingAction.SetActive(!GameFactory.Player.IsAnchored && GameFactory.Player.IsTrollingPossible);
			if (GameFactory.Player.IsEnginePresent)
			{
