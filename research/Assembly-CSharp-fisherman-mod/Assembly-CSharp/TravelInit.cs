...
public class TravelInit : MonoBehaviour
{
	...
	public void OnClickToTravel()
	{
		...
		if (this._pond.PondPaidLocked())
		{
			this.SetTravelling(false);
			if (MenuHelpers.Instance.IsInMenu)
			{
				if (MenuHelpers.Instance.MenuPrefabsList.PremiumShopRetailHandler != null)
				{
					MenuHelpers.Instance.MenuPrefabsList.PremiumShopRetailHandler.MoveToProductPondPaid(this._pond.PondId);
				}
				DashboardTabSetter.SwitchForms(FormsEnum.PremiumShopRetail, false);
			}
			return;
		}
		if (this._pond.PondLocked())
		{
			UIHelper.ShowMessage(ScriptLocalization.Get("MessageCaption"), string.Format(ScriptLocalization.Get("UnlockedAtLevelCaption"), this._pond.MinLevel), true, null, false);
			this.SetTravelling(false);
		}
