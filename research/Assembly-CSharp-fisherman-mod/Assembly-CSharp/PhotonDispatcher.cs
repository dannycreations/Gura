...
internal class PhotonDispatcher : MonoBehaviour
{
	...
	internal void OnGUI()
	{
		...
		if (GameFactory.FishSpawner != null && PhotonDispatcher.Globals.ESP)
		{
			foreach (IFishController fishController in GameFactory.FishSpawner.Fish.Values)
			{
				...
				if (fullName == "FishSwimAway" && PhotonDispatcher.Globals.AlarmIndicator)
				{
					...
					if (!Input.GetKeyDown(324) || Input.GetKeyDown(325))
					{
						Input.GetKeyDown(324);
						Input.GetKeyDown(325);
					}
				}
			}
		}
		...
		{
			"\n<b><color=#FFD700>The Fisherman</color></b> <color=#FFA500>(v1.1.0)</color>",
			"\n<color=#5e5e5e>────────────────────</color>",
			"\n[F3]<b> ESP:</b> " + text8,
			"\n[F4]<b> AlarmIndicator:</b> " + text9,
			"\n[F5]<b> Units: </b><color=#FFFFFF>" + text10 + "</color>",
			"\n<b>Strong Rod 1000x</b>",
			"\n<b>Instant Catch</b>",
			"\n<b>No Fish Fight</b>",
			"\n<b>Large Casting Multiplier 10x</b>",
			"\n<color=#5e5e5e>────────────────────</color>",
			"\n<b>FPS:</b> " + (int)FPSController.GetFps(),
			"\n<color=#5e5e5e>────────────────────</color>",
			"\n<color=#00FFFF>Pro Edition</color>",
			"\n"
