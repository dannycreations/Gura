...
public class DeluxeEyeAdaptationLogic
{
	...
	private void RenderHistogram(Material histogramMaterial)
	{
		...
		RenderTexture renderTexture = this.m_HistogramList[this.m_CurrentHistogram];
		...
		RenderTexture renderTexture2 = this.m_HistogramList[0];
		...
		histogramMaterial.SetTexture("_HistogramTex", renderTexture2);
		...
		renderTexture2.filterMode = 0;
		RenderTexture.active = this.m_BrightnessRT;
		GL.Clear(true, true, Color.black);
		Graphics.Blit(renderTexture2, this.m_BrightnessRT, histogramMaterial, 1);
	...
	public void ComputeExposure(int screenWidth, int screenHeight, Material histogramMaterial)
	{
		...
		if (this.m_PreviousBrightnessRT == null)
		{
			...
			this.m_PreviousBrightnessRT.hideFlags = 61;
	...
	public void BuildMeshes(int width, int height)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			num4 -= num8;
			Vector3[] array = new Vector3[num8 * 3];
			int[] array2 = new int[num8 * 3];
			Vector2[] array3 = new Vector2[num8 * 3];
			Vector2[] array4 = new Vector2[num8 * 3];
			Vector3[] array5 = new Vector3[num8 * 3];
			Color[] array6 = new Color[num8 * 3];
			for (int j = 0; j < num8; j++)
			{
				int num9 = num5 % width;
				int num10 = (num5 - num9) / width;
				this.SetupSprite(j, num9, num10, array, array2, array3, array4, array5, array6, new Vector2((float)num9 / (float)width + vector.x, 1f - ((float)num10 / (float)height + vector.y)), num6 * 0.5f, num7 * 0.5f);
				num5++;
			}
			mesh.vertices = array;
