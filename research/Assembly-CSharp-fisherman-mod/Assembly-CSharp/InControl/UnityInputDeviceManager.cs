...
namespace InControl
{
	public class UnityInputDeviceManager : InputDeviceManager
	{
		...
		private void DetectJoystickDevice(int unityJoystickId, string unityJoystickName)
		{
			...
			if (InputManager.UnityVersion < new VersionInfo(4, 5, 0, 0) && (Application.platform == null || Application.platform == 1) && unityJoystickName == "Unknown Wireless Controller")
			{
				return;
			}
			if (InputManager.UnityVersion >= new VersionInfo(4, 6, 3, 0) && (Application.platform == 7 || Application.platform == 2) && string.IsNullOrEmpty(unityJoystickName))
			{
				return;
			}
