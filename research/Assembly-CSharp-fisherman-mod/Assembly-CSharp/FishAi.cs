...
public class FishAi
{
	...
	public bool IsPathCompleted
	{
		get
		{
			...
			return targetPosition != null && this.CurrentBehavior != FishAiBehavior.KeepPosition && (this.mouth.Position - this.TargetPosition.Value).magnitude <= 0.25f;
		}
	}
	...
	private void UpdateAttacks(float dt, bool isTackleBeingPulled)
	{
		...
		{
			if (this.CurrentBehavior != FishAiBehavior.Bite)
			{
				if (this.CurrentBehavior == FishAiBehavior.Swim)
				{
					this._currentMaxSpeed = this.Speed;
					Vector3? targetPosition = this.TargetPosition;
				...
				{
					if (this.CurrentBehavior == FishAiBehavior.PredatorSwim)
					{
						...
						{
							float num3 = (magnitude2 - 0.1f) / 0.9f;
							num2 = num3 * 1f + (1f - num3) * 0.1f;
						}
						this.ApplyForce(this.TargetDirection.normalized * this.Force * num2, false);
						return;
					}
					if (this.CurrentBehavior == FishAiBehavior.Wait)
					{
						this.ResetForce();
			...
			float num4 = this.Force * 0.1f;
			if (this.moveForward)
			{
				this.ApplyForce(targetDirection.normalized * num4, false);
				return;
			}
			if (this.IsBiteTemplate && this.countTastes > 0 && this.biteDirection != FishBiteDirection.None)
			{
				this.ApplyForce(this.retreatDirection * num4 * this.factorForce, false);
				return;
			}
			this.ApplyForce(targetDirection.normalized * -num4, true);
			return;
		}
	}
	...
	private void TryTaste()
	{
		...
		bool isBiteTemplate = this.IsBiteTemplate;
	}
	...
	private void Strike()
	{
		...
		if (num > 0f && Random.Range(0f, 1f) < num)
		{
			...
			if (randomTrail == this.keepPositionTrail)
			{
	...
	private void ChangeBehavior(float playerForce, float dt)
	{
		...
		else
		{
			...
			if (num3 != null && this.timeSpent > 1f)
			{
				...
				float? num8 = this.priorDistanceToTarget;
	...
	private bool CheckManeuverFinished()
	{
		...
		if (!flag)
		{
			...
			if (num != null && Mathf.Abs(this.priorMouthPosition.Value - this.mouth.Position.y) / Time.deltaTime < 0.2f && Mathf.Abs(this.mouth.Position.y - -0.15f) < 0.1f)
			{
				flag = true;
			}
	...
	private void GenerateRandomBehavior(bool avoidManeouvers = false)
	{
		...
		if (!this.surfaceShocked && this.mouth.Position.y > -0.15f && this.surfaceShockTrail != null && Random.Range(0f, 1f) < this.surfaceShockTrail.Probability)
		{
			this.GoToSurfaceShock();
			return;
		}
		if (this.CurrentBehavior == FishAiBehavior.Shocked && this.initialJerkTrail != null && Random.Range(0f, 1f) < this.initialJerkTrail.Probability)
		{
			this.GoToInitialJerk();
			return;
		}
		if (Random.Range(0f, this.allEscapesProbability + this.allManeuversProbability) < this.allEscapesProbability || this.CurrentRelativeForce < 0.5f || avoidManeouvers || !this.isLineTensioned || this.areManeouversDisabled)
		{
			this.CurrentBehavior = FishAiBehavior.Escape;
			this.GenerateRandomEscape();
			return;
		...
		if (randomTrail == this.headShakeTrail)
		{
			this.GoToHeadShake();
			return;
		}
		if (randomTrail == this.jumpOutTrail)
		{
			this.GoToJumpOut();
			return;
		}
		if (randomTrail == this.candleTrail)
		{
			this.GoToCandle();
		}
	}
	...
	private void UpdateStamina()
	{
		...
		if (this.CurrentBehavior == FishAiBehavior.HeadShake)
		{
			this._currentMaxSpeed = global::FishAi.MaxHeadShakeSpeed;
			return;
		}
		if (this.CurrentBehaviorDirection == FishBehaviorDirection.To && (this.CurrentBehavior == FishAiBehavior.Escape || this.CurrentBehavior == FishAiBehavior.Float || this.CurrentBehavior == FishAiBehavior.Sink))
		{
	...
	{
		float num = Math3d.AngleSigned((this.mouth.Position - this.root.Position).normalized, force.normalized, Vector3.up);
	...
	private bool HasObstaclesToPlayerLookingFromWaterSurface(Vector3 point)
	{
		...
		Vector3 vector2 = new Vector3(this.playerPosition.x, 1f, this.playerPosition.z) - vector;
		RaycastHit raycastHit;
		return Physics.Raycast(vector, vector2, ref raycastHit, float.PositiveInfinity, GlobalConsts.FishMask) && raycastHit.distance < vector2.magnitude - 1f;
	}
	...
	private FishBitePart[] _biteParts
	{
		get
		{
			if (this._portrait.ResultFishBiteTemplate != null)
			{
				return this._portrait.ResultFishBiteTemplate.ResultParts;
			}
			return null;
		}
	}
	...
	private FishBiteKeepIn[] _biteKeepIns
	{
		get
		{
			if (this._portrait.ResultFishBiteTemplate != null)
			{
				return this._portrait.ResultFishBiteTemplate.ResultKeepIn;
			}
			return null;
		}
	}
	...
	private void ApplyCurrentBehavior()
	{
		...
		if (targetPosition != null)
		{
			...
			case FishAiBehavior.Pool:
			{
				...
				{
					if ((this.pathCheckPosition - this.mouth.Position).magnitude < 0.5f * this.Speed * 0.2f)
					{
						this.GenerateEscapePointForPool();
					}
		...
		if (this.CurrentBehavior == FishAiBehavior.Eights)
		{
