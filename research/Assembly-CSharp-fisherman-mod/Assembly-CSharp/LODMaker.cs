...
public static class LODMaker
{
	...
	public static Mesh MakeLODMesh(Mesh orig, float aMaxWeight, float removeSmallParts, float protectNormals, float protectUvs, float smallTrianglesFirst, float protectSubMeshesAndSharpEdges, bool recalcNormals, bool reuseOldMesh = false)
	{
		...
		float num7;
		float num8;
		LODMaker.GetWeights(aMaxWeight, removeSmallParts, protectNormals, protectUvs, smallTrianglesFirst, protectSubMeshesAndSharpEdges, out num, out num2, out num3, out num4, out num5, out num6, out num7, out num8);
		return LODMaker.MakeLODMesh(orig, aMaxWeight, removeSmallParts, num, num2, num3, num4, num5, num6, num7, num8, recalcNormals, reuseOldMesh);
	}
	...
	public static void MakeLODMeshInBackground(object data)
	{
		...
		int[] array3 = (int[])hashtable["subMeshOffsets"];
		Bounds bounds = (Bounds)hashtable["meshBounds"];
		float num2 = 1f;
		float num3 = 1f;
		float num4 = 1f;
		float num5 = 1f;
		...
		Vector2[] array8 = null;
		Color32[] array9 = null;
		Matrix4x4[] array10 = null;
		BoneWeight[] array11 = null;
		if (hashtable.ContainsKey("removeSmallParts"))
		{
			num2 = (float)hashtable["removeSmallParts"];
		}
		if (hashtable.ContainsKey("protectNormals"))
		{
			num3 = (float)hashtable["protectNormals"];
		}
		if (hashtable.ContainsKey("protectUvs"))
		{
			num4 = (float)hashtable["protectUvs"];
		}
		if (hashtable.ContainsKey("protectSubMeshesAndSharpEdges"))
		{
			num5 = (float)hashtable["protectSubMeshesAndSharpEdges"];
		}
		if (hashtable.ContainsKey("smallTrianglesFirst"))
		{
			num6 = (float)hashtable["smallTrianglesFirst"];
		}
		if (hashtable.ContainsKey("normals"))
		{
			array4 = (Vector3[])hashtable["normals"];
		}
		if (hashtable.ContainsKey("uv1s"))
		{
			array5 = (Vector2[])hashtable["uv1s"];
		}
		if (hashtable.ContainsKey("uv2s"))
		{
			array6 = (Vector2[])hashtable["uv2s"];
		}
		if (hashtable.ContainsKey("uv3s"))
		{
			array7 = (Vector2[])hashtable["uv3s"];
		}
		if (hashtable.ContainsKey("uv4s"))
		{
			array8 = (Vector2[])hashtable["uv4s"];
		}
		if (hashtable.ContainsKey("colors32"))
		{
			array9 = (Color32[])hashtable["colors32"];
		}
		if (hashtable.ContainsKey("bindposes"))
		{
			array10 = (Matrix4x4[])hashtable["bindposes"];
		}
		if (hashtable.ContainsKey("boneWeights"))
		{
			array11 = (BoneWeight[])hashtable["boneWeights"];
		}
		float num7;
		float num8;
		float num9;
		...
		float num13;
		float num14;
		LODMaker.GetWeights(num, num2, num3, num4, num6, num5, out num7, out num8, out num9, out num10, out num11, out num12, out num13, out num14);
		List<Vector3> list;
		List<Vector3> list2;
		List<Vector2> list3;
		List<Vector2> list4;
		...
		LODMaker.MakeLODMesh(array, array4, array5, array6, array7, array8, array9, array2, ref array10, array11, ref array3, bounds, num, num2, num7, num8, num9, num10, num11, num12, num13, num14, out list, out list2, out list3, out list4, out list5, out list6, out list7, out list8, out list9);
		...
		((Hashtable)data)["bindposes"] = array10;
		((Hashtable)data)["boneWeights"] = list9.ToArray();
		((Hashtable)data)["subMeshOffsets"] = array3;
	...
	private static Mesh MakeLODMesh(Mesh orig, float maxWeight, float removeSmallParts, float sideLengthWeight, float oldAngleWeight, float newAngleWeight, float uvWeight, float areaDiffWeight, float normalWeight, float vertexWeight, float centerDistanceWeight, bool recalcNormals, bool reuseOldMesh)
	{
		...
		int[] array2 = new int[orig.subMeshCount];
		if (orig.subMeshCount > 1)
		{
			for (int i = 0; i < orig.subMeshCount; i++)
			{
				...
				{
					triangles[array2[i] + j] = triangles2[j];
				}
				if (i + 1 < orig.subMeshCount)
				{
					array2[i + 1] = array2[i] + j;
				}
			}
		}
		...
		if (reuseOldMesh)
		{
			...
			orig.subMeshCount = 0;
			orig.normals = null;
			LODMaker.FillMesh(orig, list.ToArray(), list2.ToArray(), list3.ToArray(), list4.ToArray(), list5.ToArray(), list6.ToArray(), list7.ToArray(), list8.ToArray(), list9.ToArray(), bindposes, array2, recalcNormals);
			return orig;
		}
		return LODMaker.CreateNewMesh(list.ToArray(), list2.ToArray(), list3.ToArray(), list4.ToArray(), list5.ToArray(), list6.ToArray(), list7.ToArray(), list8.ToArray(), list9.ToArray(), bindposes, array2, recalcNormals);
	}
	...
	private static void MakeLODMesh(Vector3[] vs, Vector3[] ns, Vector2[] uv1s, Vector2[] uv2s, Vector2[] uv3s, Vector2[] uv4s, Color32[] colors32, int[] ts, ref Matrix4x4[] bindposes, BoneWeight[] bws, ref int[] subMeshOffsets, Bounds meshBounds, float maxWeight, float removeSmallParts, float sideLengthWeight, float oldAngleWeight, float newAngleWeight, float uvWeight, float areaDiffWeight, float normalWeight, float vertexWeight, float centerDistanceWeight, out List<Vector3> newVs, out List<Vector3> newNs, out List<Vector2> newUv1s, out List<Vector2> newUv2s, out List<Vector2> newUv3s, out List<Vector2> newUv4s, out List<Color32> newColors32, out List<int> newTs, out List<BoneWeight> newBws)
	{
		...
		{
			int num8 = LODMaker.GetLastVertexWithYSmaller(vs[num7].y, list3, vs, num7);
			for (num8++; num8 < list3.Count; num8++)
			{
				if (list3[num8] < 0)
				{
					break;
				}
				if (vs[list3[num8]].y > vs[num7].y)
				{
					break;
				}
				if (vs[list3[num8]].y == vs[num7].y)
				{
					if (vs[list3[num8]].z > vs[num7].z)
					{
						break;
					}
					if (vs[num7].z == vs[list3[num8]].z && vs[list3[num8]].x > vs[num7].x)
					{
						break;
					}
				}
			}
			list3.Insert(num8, num7);
		}
		int num9 = -1;
		...
		for (int num12 = 0; num12 < vs.Length; num12++)
		{
			...
			List<int> verticesEqualTo = LODMaker.GetVerticesEqualTo(vs[num12], list3, vs);
		...
		{
			list2[array7[ts[num15]]].Add(num15 / 3);
		}
		...
		int[] array14 = new int[] { 1, 0, 2, 1, 0, 2 };
		int[] array15 = new int[] { 0, 1, 1, 2, 2, 0 };
		for (int num23 = 0; num23 < 3; num23++)
		{
			...
			for (int num26 = 0; num26 < ts.Length; num26 += 3)
			{
				...
				{
					int[] array16 = new int[] { num27, num28, num28, num29, num29, num27 };
					int[] array17 = new int[] { num28, num27, num29, num28, num27, num29 };
					...
					if (num30 >= num24 && num30 < num25)
					{
						...
						int[] adjacentTriangles = LODMaker.GetAdjacentTriangles(ts, num26, list2, array7, array, list);
								...
								if (LODMaker.AnyWeightOK(array18, num16))
								{
									...
									for (int num42 = 0; num42 < 6; num42++)
									{
										if (array18[num42] < num16)
										{
											...
											float num45 = 0f;
											float num46 = 0f;
											LODMaker.GetUVStretchAndAreaForCorner(adjacentTriangles, vs, array6, array7, uv1s, array16[num42], array17[num42], ref num44, ref num43, ref num46, ref num45);
											array18[num42] += num44 * 10f * uvWeight;
											array18[num42] += (Mathf.Pow(Mathf.Abs(num46) + 1f, 2f) - 1f) * 30f * uvWeight;
											if (num41 <= 0f)
											{
						...
						if (LODMaker.AnyWeightOK(array18, num16))
						{
							...
							{
								array18[num47] *= 0.05f + 0.95f * array19[num47 / 2];
							}
							int num48 = -1;
							float num49 = float.PositiveInfinity;
							for (int num50 = 0; num50 < 6; num50++)
							{
								if (array18[num50] < num49)
								{
									num49 = array18[num50];
									num48 = num50;
								}
							...
							{
							case 0:
								LODMaker.MergeVertices(ref num27, num28, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							case 1:
								LODMaker.MergeVertices(ref num28, num27, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							case 2:
								LODMaker.MergeVertices(ref num28, num29, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							case 3:
								LODMaker.MergeVertices(ref num29, num28, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							case 4:
								LODMaker.MergeVertices(ref num29, num27, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							case 5:
								LODMaker.MergeVertices(ref num27, num29, array5, vs, ts, uv1s, uv2s, uv3s, uv4s, colors32, array4, array6, array7, array8, array9, array10, array11, array12, list2, num > 1);
								break;
							}
						}
					}
		...
		int[] array22 = new int[vs.Length];
		LODMaker.FillNewMeshArray(vs, array4, array6, ns, uv1s, array8, uv2s, array9, uv3s, array10, uv4s, array11, colors32, array12, bws, newVs, newNs, newUv1s, newUv2s, newUv3s, newUv4s, newColors32, newBws, array22);
		LODMaker.FillNewMeshTriangles(ts, array22, newTs, subMeshOffsets, array, list4);
		LODMaker.RemoveEmptyTriangles(newVs, newNs, newUv1s, newUv2s, newUv3s, newUv4s, newColors32, newTs, newBws, subMeshOffsets, list4);
		if (removeSmallParts > 0f)
		{
			LODMaker.RemoveMiniTriangleGroups(removeSmallParts, zero, maxWeight, newVs, newTs, subMeshOffsets, list4);
		}
			...
			if (list5.Count > 0)
			{
				if (num > 0)
				{
					for (int m = 0; m < list4.Count; m++)
					{
						List<int> list6;
						int num2;
						(list6 = subMeshes[list4[m]])[num2 = list5[m]] = list6[num2] - num;
					}
				}
			}
			else
			...
			if (list5.Count > 0)
			{
				if (num > 0)
				{
					for (int l = 0; l < list4.Count; l++)
					{
						List<int> list6;
						int num2;
						(list6 = subMeshes[list4[l]])[num2 = list5[l]] = list6[num2] - num;
					}
				}
			}
			else
	...
	{
		return (p1 - p0).magnitude * (Mathf.Sin(Vector3.Angle(p1 - p0, p2 - p0) * 0.017453292f) * (p2 - p0).magnitude) * 0.5f;
	}
						...
						if (ts[num] != i && ts[num + 1] != i && ts[num + 2] != i)
						{
							if (!hiddenByOtherSubmesh || subMeshIdxPerVertex[ts[num]] != subMeshIdxPerVertex[i])
							{
								...
								if (num2 < 60f)
								{
									...
									{
										Vector3 vector = vertex + normal * num3;
										Vector2 vector2 = vector.Barycentric(vs[ts[num]], vs[ts[num + 1]], vs[ts[num + 2]]);
										if (vector2.IsBarycentricInTriangle())
										{
											if (vObscured != null)
											{
	...
	public static float FindCollision(Vector3 fromPos, Vector3 direction, Vector3 pointOnPlane, Vector3 normalPlane)
	{
		...
		float num2 = direction.InProduct(normalPlane);
		if (num2 != 0f)
		{
			num = (pointOnPlane - fromPos).InProduct(normalPlane) / direction.InProduct(normalPlane);
		}
		return num;
	}
