...
public class LODSwitcher : MonoBehaviour
{
	private void Start()
	{
		this.frameOffset = Mathf.RoundToInt(Random.value * 10f);
	...
	public void ComputeDimensions()
	{
		...
		Vector3 vector = main.ScreenToWorldPoint(new Vector3(((float)Screen.width - 100f) / 2f, 0f, 1f));
		Vector3 vector2 = main.ScreenToWorldPoint(new Vector3(((float)Screen.width + 100f) / 2f, 0f, 1f));
		this.pixelsPerMeter = 1f / (Vector3.Distance(vector, vector2) / 100f);
	}
	...
	public Vector3 NearestCameraPositionForLOD(int aLevel)
	{
		...
		if (aLevel > 0 && aLevel <= this.lodScreenSizes.Length)
		{
			...
			return base.transform.position + this.centerOffset + main.transform.rotation * (Vector3.back * num2);
		}
