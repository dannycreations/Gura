...
public class DeluxeTonemapper : MonoBehaviour
{
	...
	public void ClearLutTextures()
	{
		...
		{
			Object.DestroyImmediate(this.m_3DLut0);
			this.m_3DLut0 = null;
		}
		this.m_2DLut0 = null;
	}
	...
	public void UpdateCoefficients()
	{
		...
		float num = ((QualitySettings.activeColorSpace != 1) ? 1f : 0.5f);
		if (this.m_Use2DLut && this.m_2DLut0 != null)
		{
			this.m_2DLut0.filterMode = 1;
			float num2 = Mathf.Sqrt((float)this.m_2DLut0.width);
			this.m_Material.SetTexture("_2dLut0", this.m_2DLut0);
			this.m_Material.SetVector("_Lut0Params", new Vector4(this.m_Lut0Amount, 1f / (float)this.m_2DLut0.width, 1f / (float)this.m_2DLut0.height, num2 - 1f));
			this.m_Material.SetFloat("_IsLinear", num);
		}
		else if (this.m_3DLut0 != null)
		{
			int width = this.m_3DLut0.width;
			float num3 = (float)(width - 1) / (1f * (float)width);
			float num4 = 1f / (2f * (float)width);
			this.m_3DLut0.filterMode = 2;
			this.m_Material.SetVector("_Lut0Params", new Vector4(this.m_Lut0Amount, num3, num4, 0f));
