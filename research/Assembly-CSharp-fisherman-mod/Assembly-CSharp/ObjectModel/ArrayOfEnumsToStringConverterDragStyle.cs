...
{
	public class ArrayOfEnumsToStringConverterDragStyle : JsonConverter
	{
		...
		public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
		{
			...
			List<DragStyle> list = new List<DragStyle>();
			while (reader.TokenType != 14)
			{
				...
				{
					list.Add((DragStyle)Enum.Parse(typeof(DragStyle), reader.Value.ToString()));
				}
				else if (reader.TokenType == 7)
				{
					list.Add((DragStyle)Enum.ToObject(typeof(DragStyle), (long)reader.Value));
				}
			}
			return list.ToArray();
		}
