	...
	public class RodCaster
	{
		public float CurrentMaxCastLength
		{
			get
			{
				...
				{
					num = this.RodMaxCastLength * 1f;
				}
				else if (this.TackleWeight <= this.AbsoluteX2)
				{
					num = RodCaster.Interpolate(this.AbsoluteX1, this.RodMaxCastLength * 1f, this.AbsoluteX2, this.RodMaxCastLength * 6f, this.TackleWeight);
				}
				else if (this.TackleWeight <= this.RodCastWeightMin)
				{
					num = RodCaster.Interpolate(this.AbsoluteX2, this.RodMaxCastLength * 6f, this.RodCastWeightMin, this.RodMaxCastLength * 8.5f, this.TackleWeight);
				}
				else if (this.TackleWeight <= this.Peak)
				{
					num = RodCaster.Interpolate(this.RodCastWeightMin, this.RodMaxCastLength * 8.5f, this.Peak, this.RodMaxCastLength, this.TackleWeight);
				}
				else if (this.TackleWeight <= this.RodCastWeightMax)
				{
					num = RodCaster.Interpolate(this.Peak, this.RodMaxCastLength, this.RodCastWeightMax, this.RodMaxCastLength * 9.5f, this.TackleWeight);
				}
				else if (this.TackleWeight <= this.AbsoluteX3)
				{
					num = RodCaster.Interpolate(this.RodCastWeightMax, this.RodMaxCastLength * 9.5f, this.AbsoluteX3, this.RodMaxCastLength * 7f, this.TackleWeight);
				}
				else if (this.TackleWeight <= this.AbsoluteX4)
				{
					num = RodCaster.Interpolate(this.AbsoluteX3, this.RodMaxCastLength * 7f, this.AbsoluteX4, this.RodMaxCastLength * 3f, this.TackleWeight);
				}
				else
				{
					num = this.RodMaxCastLength * 3f;
				}
				num *= this.ReelFriction * this.TackleWindage * ((this.LineType != ItemSubTypes.BraidLine) ? 10f : 9.5f) * this.LineThicknessEffect * this.CastingForce;
				return Math.Max(this.RodLength * 20f, num);
			}
		}
