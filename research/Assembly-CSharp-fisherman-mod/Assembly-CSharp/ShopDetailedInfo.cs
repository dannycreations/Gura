...
public class ShopDetailedInfo : MonoBehaviour
{
	...
	public void Show(ShopLicense item, int term)
	{
		...
		if (this.lastShowLicenseId != item.LicenseId)
		{
			...
			this.Name.text = item.Name;
			this.DurationNote.text = ScriptLocalization.Get("LevelRequirements") + item.OriginalMinLevel;
			Text priceLabel = this.PriceLabel;
			text = string.Empty;
			this.LicenseRequirements.text = text;
		...
		Text expires = this.Expires;
		text = string.Empty;
		this.LicenseDuration.text = text;
		expires.text = text;
		...
		text = ((!(shopLicenseCost.Currency != "GC")) ? "\ue62c" : "\ue62b");
