...
public class FishBite : FishStateBase
{
	...
	protected override Type onUpdate()
	{
		...
		if (!this.renewedBiteAge)
		{
			this.biteAge = 0f;
			this.renewedBiteAge = true;
		}
		this.biteAge += Time.deltaTime;
		if (this.biteAge > this.biteTotalTime)
		{
			return typeof(FishSwimAway);
		}
		if (!this._isBiteRequestSend)
		{
			this._isBiteRequestSend = true;
			base.Adapter.ConfirmFishBite(base.Fish.InstanceGuid);
			this.biteTotalTime = 1f;
			this.biteAge = 0f;
			RandomSounds.PlaySoundAtPoint("Sounds/incoming_bite3", GameFactory.Player.Position, 10f, false);
		}
		return null;
	}
