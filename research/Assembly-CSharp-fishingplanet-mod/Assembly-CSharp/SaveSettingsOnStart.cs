...
public class SaveSettingsOnStart : ActivityStateControlled
{
	private void Awake()
	{
		...
		this._resetProfile.SetActive(false);
	...
	{
		PhotonConnectionFactory.Instance.OnPreviewQuitComplete += this.OnPreviewQuitComplete;
		PhotonConnectionFactory.Instance.OnPreviewQuitFailed += this.OnPreviewQuitFailed;
		PhotonConnectionFactory.Instance.GracefulDisconnect();
	}

	private void OnPreviewQuitFailed(Failure f)
	{
		PhotonConnectionFactory.Instance.OnPreviewQuitComplete -= this.OnPreviewQuitComplete;
		PhotonConnectionFactory.Instance.OnPreviewQuitFailed -= this.OnPreviewQuitFailed;
		Debug.LogError(f.FullErrorInfo);
		this.Confirm_ActionCalled(null, null);
	}

	private void OnPreviewQuitComplete()
	{
		PhotonConnectionFactory.Instance.OnPreviewQuitComplete -= this.OnPreviewQuitComplete;
		PhotonConnectionFactory.Instance.OnPreviewQuitFailed -= this.OnPreviewQuitFailed;
