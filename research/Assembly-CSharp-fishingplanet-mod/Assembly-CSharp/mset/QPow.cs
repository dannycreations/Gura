...
namespace mset
{
	public class QPow
	{
		...
		public static QPow.PowFunc closestPowFunc(int exp)
		{
			if (exp + 128 >= 512)
			{
				return new QPow.PowFunc(QPow.Pow512);
			}
			if (exp + 64 >= 256)
			{
				return new QPow.PowFunc(QPow.Pow256);
			}
			if (exp + 32 >= 128)
			{
				return new QPow.PowFunc(QPow.Pow128);
			}
			if (exp + 16 >= 64)
			{
				return new QPow.PowFunc(QPow.Pow64);
			}
			if (exp + 8 >= 32)
			{
				return new QPow.PowFunc(QPow.Pow32);
			}
			if (exp + 4 >= 16)
			{
				return new QPow.PowFunc(QPow.Pow16);
			}
			if (exp + 2 >= 8)
			{
				return new QPow.PowFunc(QPow.Pow8);
			}
			if (exp + 1 >= 4)
			{
				return new QPow.PowFunc(QPow.Pow4);
			}
			if (exp >= 2)
			{
				return new QPow.PowFunc(QPow.Pow2);
			}
			return new QPow.PowFunc(QPow.Pow1);
		}
