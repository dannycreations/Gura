				...
				else
				{
					for (int j = 0; j < characterCount; j++)
					{
						...
						if (tmp_CharacterInfo.isVisible)
						{
							...
							Vector3 vector3;
							vector3..ctor(Random.Range(-0.25f, 0.25f), Random.Range(-0.25f, 0.25f), 0f);
							Matrix4x4 matrix = Matrix4x4.TRS(vector3 * this.CurveScale, Quaternion.Euler(0f, 0f, Random.Range(-5f, 5f) * this.AngleMultiplier), Vector3.one);
