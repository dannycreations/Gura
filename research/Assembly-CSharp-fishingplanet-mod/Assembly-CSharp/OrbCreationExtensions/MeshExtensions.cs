...
namespace OrbCreationExtensions
{
	public static class MeshExtensions
	{
		...
		{
			Mesh mesh = Object.Instantiate<Mesh>(orig);
		...
		public static Mesh CopyAndRemoveHiddenTriangles(this Mesh orig, int subMeshIdx, Matrix4x4 localToWorldMatrix, Mesh[] hidingMeshes, int[] hidingSubMeshes, Matrix4x4[] hidingLocalToWorldMatrices, float maxRemoveDistance = 0.01f)
		{
			...
			{
				Vector3 vector4 = localToWorldMatrix.MultiplyPoint3x4(list2[list11[n]]);
				Vector3 vector5 = localToWorldMatrix.MultiplyPoint3x4(list2[list11[n + 1]]);
				Vector3 vector6 = localToWorldMatrix.MultiplyPoint3x4(list2[list11[n + 2]]);
				if (!MeshExtensions.IsTriangleHidden(vector4, vector5, vector6, maxRemoveDistance, list14, list15, list12, list13))
				{
					list16.Add(list11[n]);
		...
		private static bool IsHidden(Vector3 v, Vector3 n, float maxDistance, List<Vector3> hidingVs, List<int> hidingTs, List<int> trianglesToCheck)
		{
			for (int i = 0; i < trianglesToCheck.Count; i++)
			{
				...
				if (num2 < 60f)
				{
					...
					{
						Vector3 vector4 = v + n * num3;
						Vector2 vector5 = vector4.Barycentric(vector, vector2, vector3);
						if (vector5.IsBarycentricInTriangle())
						{
							return true;
						}
					}
		...
		private static List<int> GetTrianglesWithinRange(Vector3 v0, Vector3 v1, Vector3 v2, float maxDistance, List<Vector3> triMinCorners, List<Vector3> triMaxCorners)
		{
			List<int> list = new List<int>();
			Vector3 vector;
			vector..ctor(Mathf.Min(Mathf.Min(v0.x, v1.x), v2.x) - maxDistance, Mathf.Min(Mathf.Min(v0.y, v1.y), v2.y) - maxDistance, Mathf.Min(Mathf.Min(v0.z, v1.z), v2.z) - maxDistance);
			Vector3 vector2;
			vector2..ctor(Mathf.Max(Mathf.Max(v0.x, v1.x), v2.x) + maxDistance, Mathf.Max(Mathf.Max(v0.y, v1.y), v2.y) + maxDistance, Mathf.Max(Mathf.Max(v0.z, v1.z), v2.z) + maxDistance);
