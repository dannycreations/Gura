...
[ExecuteInEditMode]
public class LakeWaterRipples : MonoBehaviour
{
	...
	private void Update()
	{
		...
		for (int l = 1; l < this.ripplesGridSize - 1; l++)
		{
			for (int m = 1; m < this.ripplesGridSize - 1; m++)
			{
				...
				this.ripplesHeightMap[num3].a = (byte)(LakeWaterRipples.Clamp(this.vertSpeed[num3]) * 127f + 128f);
			}
		}
		for (int n = 0; n < this.ripplesGridSize; n++)
		{
			for (int num5 = 0; num5 < this.ripplesGridSize; num5++)
			{
				...
				this.ripplesHeightMap[num6].g = (byte)(LakeWaterRipples.Clamp(this.heightCur[num6] * this.vertScale) * 127f + 127f);
				this.ripplesHeightMap[num6].r = (byte)(LakeWaterRipples.Clamp(this.heightCur[(num6 + 1) % (this.ripplesGridSize * this.ripplesGridSize)] - this.heightCur[num6]) * 127f + 127f);
				this.ripplesHeightMap[num6].b = (byte)(LakeWaterRipples.Clamp(this.heightCur[(num6 + this.ripplesGridSize) % (this.ripplesGridSize * this.ripplesGridSize)] - this.heightCur[num6]) * 127f + 127f);
			}
		}
	...
	public int ripplesGridSize = 512;
