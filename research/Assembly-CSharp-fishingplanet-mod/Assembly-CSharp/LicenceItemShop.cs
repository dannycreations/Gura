...
public class LicenceItemShop : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		this._terms = new Dictionary<Text, int>
		{
			{ this.DayCost, 1 },
			{ this.ThreeDayCost, 3 },
			{ this.WeekCost, 7 },
	...
	public void FillData(EventHandler<EventArgs> onChoose)
	{
		...
		using (Dictionary<Text, int>.Enumerator enumerator2 = this._terms.GetEnumerator())
		{
			while (enumerator2.MoveNext())
			{
				KeyValuePair<Text, int> term = enumerator2.Current;
				if (term.Value != 0)
				{
					term.Key.transform.parent.gameObject.GetComponent<HintElementId>().SetElementId(string.Format("{0}_{1}", text, term.Value), null, null);
				}
				this.SetCost(this.Licence.Costs.FirstOrDefault((ShopLicenseCost x) => x.Term == term.Value), term.Key.transform, flag);
			}
		}
